...
SIMULATED_AI = os.getenv("SIMULATED_AI", "false").lower() in {
    "1",
    "true",
    "yes",
    "on",
...
class Handler(BaseHTTPRequestHandler):
            ...
            return self._json(
                200,
                {
                    ...
                    "details": {
                        ...
                        "simulated": SIMULATED_AI and not bool(UPSTREAM_URL),
                    },
                },
            )
    ...
    def do_POST(self):
        ...
        if not UPSTREAM_URL:
            if SIMULATED_AI:
                # Local demo only. These boxes are made up, and are
                # marked so, because a demo that looks like a model
                # is how a fake ships to a customer.
                return self._json(
                    200,
                    {
                        "plugin_id": PLUGIN_ID,
                        ...
                        "metadata": {
                            "mode": "simulated",
                            "warning": "Demo output; no AI model executed",
                        },
                    },
                )
            # No model wired yet. Returning nothing proves the path
            ...
            return self._json(
                200,
                {
                    ...
                    "metadata": {
                        "mode": "noop",
                        "message": "Set UPSTREAM_AI_URL to connect your model",
                    },
                },
            )
        ...
        try:
            ...
            # Upstream may already speak the contract. If it does not,
            # the raw body goes to metadata rather than being guessed at.
