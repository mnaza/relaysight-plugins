...
MANIFEST = {
    ...
    "description": "Presigned URLs for S3, MinIO, Backblaze B2 and friends.",
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/v1/storage/uploads":
                key = object_key(
                    body.get("namespace", "default"), body["object_key"]
                )
                expires = int(body.get("expires_seconds", 900))
                params = {
                    "Bucket": BUCKET,
                    "Key": key,
                    "ContentType": body.get(
                ...
                return self._json(
                    200,
                    transfer(
                        "PUT", url, key, expires, upload_headers, audience
                    ),
                )
            if self.path == "/v1/storage/downloads":
                ...
                return self._json(
                    200, transfer("GET", url, key, expires, {}, audience)
                )
